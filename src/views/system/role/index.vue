<template>
  <div class="ele-body">
        ...
        <el-row :gutter="15">
          ...
          <el-col :md="9" :sm="12">
            <div class="ele-form-actions">
              ...
              <el-button @click="remove()" type="danger" icon="el-icon-delete" class="ele-btn-icon" v-if="permission.includes('sys:role:delete')">批量删除</el-button>
            </div>
          </el-col>
        </el-row>
      </el-form>
        ...
        <template slot-scope="{index}">
          ...
          <el-table-column label="操作" width="230px" align="center" :resizable="false"  fixed="right">
            <template slot-scope="{row}">
              <el-link @click="edit(row)" icon="el-icon-edit" type="primary" :underline="false" v-if="permission.includes('sys:role:update')">修改</el-link>
              <el-link @click="auth(row)" icon="el-icon-finished" type="primary" :underline="false" v-if="permission.includes('sys:rolemenu:update')">分配权限</el-link>
    ...
    <!-- 权限分配弹窗 -->
    <el-dialog title="分配权限" :visible.sync="showAuth" width="400px" @closed="editForm={}" :destroy-on-close="true" :close-on-click-modal="false"
               :lock-scroll="false">
...
export default {
  ...
  data() {
    return {
      table: {url: '/role/list', where: {}},  // 表格配置
      choose: [],  // 表格选中数据
    ...
    remove(row) {
      if (!row) {  // 批量删除
        ...
        this.$confirm('确定要删除选中的角色吗?', '提示', {type: 'warning'}).then(() => {
          ...
          this.$http.delete('/role/delete/' + ids.toString()).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: 'success', message: res.data.msg});
              this.$refs.table.reload();
